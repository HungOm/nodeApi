...
handlers._users.get = function (data, callback) {
  ...
  if (phone) {
    ...
    // letify if the given token is valid for the phone number
    handlers._tokens.letifyToken(token, phone, function (tokenIsValid) {
      if (tokenIsValid) {
        _data.read("users", phone, function (err, data) {
          if (!err && data) {
...
handlers._tokens.post = function (data, callback) {
  ...
  if (phone && password) {
    ...
    performance.mark('Beginning user look up')
    _data.read("users", phone, function (err, userData) {
      performance.mark('User look up complete')
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          _data.create("tokens", tokenId, tokenObject, function (err) {
            ...
            performance.measure('User lookup', 'Beginning user lookup', 'User look up complete');
...
handlers._tokens.letifyToken = function (id, phone, callback) {
  _data.read("tokens", id, function (err, tokenData) {
    if (!err && tokenData) {
    ...
    _data.read('tokens', token, function (err, tokenData) {
      if (!err && tokenData) {
        ...
        _data.read('users', userPhone, function (err, userData) {
          if (!err && userData) {
            let userChecks = typeof (userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
            //letify the user has less than the max-check-per-user
            if (userChecks.length < config.maxChecks) {
              //create a random id for the checks
              let checkId = helpers.createRandomString(20);
              // letify the URL given has dns entry( and therfore can resolve);
              ...
              dns.resolve(hostName, function (err, records) {
                if (!err && records) {
                  ...
                  callback(400, {
                    "Error": "The host name of the url entered did not resoved to any DNS entries."
                  })
                }
              })
            } else {
  ...
  if (id) {
    ...
    _data.read('checks', id, function (err, checkData) {
      if (!err && checkData) {
        let token =
          typeof data.headers.token == "string" ? data.headers.token : false;
        // letify if the given token is valid and belongs to the user who created the check
        handlers._tokens.letifyToken(token, checkData.userPhone, function (tokenIsValid) {
          if (tokenIsValid) {
  ...
  if (id) {
    ...
    _data.read('checks', id, function (err, checkData) {
      if (!err && checkData) {
        let token =
          typeof data.headers.token == "string" ? data.headers.token : false;
        // letify if the given token is valid for the id number
        handlers._tokens.letifyToken(token, checkData.userPhone, function (tokenIsValid) {
          if (tokenIsValid) {
